...
dirs = [plugin_config.config_path,
        plugin_config.template_path,
        plugin_config.words_contents_path,
        plugin_config.res_path,
        plugin_config.re_img_path,
        plugin_config.stop_words_path,
        plugin_config.wordcloud_bg_path,
        plugin_config.user_violation_info_path,
        plugin_config.group_message_data_path,
        plugin_config.error_path]
...
async def init():
    ...
    if not plugin_config.config_admin.exists():
        await mk('file', plugin_config.config_admin, 'w', content='{"1008611": ["This_is_an_example"]}')
    if not plugin_config.config_group_admin.exists():
        await mk('file', plugin_config.config_group_admin, 'w', content='{"su": "True"}')
    if not plugin_config.word_path.exists():
        await mk('file', plugin_config.word_path, 'w', content='123456789\n')
    if not plugin_config.switcher_path.exists():
        bot = nonebot.get_bot()
        logger.info('创建开关配置文件,分群设置和违禁词检测,防撤回,广播,早晚安默认关,其他默认开')
        g_list = (await bot.get_group_list())
        switcher_dict = {}
        for group in g_list:
            switchers = {}
            for fn_name in plugin_config.admin_funcs:
                switchers.update({fn_name: True})
                if fn_name in ['auto_ban', 'group_msg', 'particular_e_notice', 'group_recall']:
                    switchers.update({fn_name: False})
            switcher_dict.update({str(group['group_id']): switchers})
        with open(plugin_config.switcher_path, 'w', encoding='utf-8') as swp:
            swp.write(f"{json.dumps(switcher_dict)}")
    if not plugin_config.black_list_path.exists():
        data = dict()
        with open(plugin_config.black_list_path, 'w', encoding='utf-8') as f:
            f.write(f"{json.dumps(data)}")
    if not plugin_config.limit_word_path.exists():  # 要联网的都丢最后面去
        if (plugin_config.config_path / '违禁词_简单.txt').exists():
            with open(plugin_config.config_path / '违禁词_简单.txt', 'r', encoding='utf-8') as f:
                content = f.read()
            with open(plugin_config.limit_word_path, 'w', encoding='utf-8') as f:
                f.write(content)
            (plugin_config.config_path / '违禁词_简单.txt').unlink()
        else:
            await mk('file', plugin_config.limit_word_path, 'w',
                     url='https://fastly.jsdelivr.net/gh/yzyyz1387/nwafu/f_words/f_word_easy', dec='简单违禁词词库')
    if not plugin_config.ttf_path.exists():
        await mk('file', plugin_config.ttf_name, 'wb', url='https://fastly.jsdelivr.net/gh/yzyyz1387/blogimages/msyhblod.ttf',
                 dec='资源字体')
...
async def get_user_violation(gid: int, uid: int, label: str, content: str, add_: bool = True) -> int:
    ...
    :return: 违规等级
    """
    path_grop = plugin_config.user_violation_info_path / f"{str(gid)}"
    path_user = path_grop / f"{str(uid)}.json"
    this_time = str(datetime.datetime.now()).replace(' ', '-')
    uid = str(uid)
    if not os.path.exists(plugin_config.user_violation_info_path):
        await mk('dir', plugin_config.user_violation_info_path, mode=None)
...
async def error_log(gid: int, time: str, matcher: Matcher, err: str) -> None:
    ...
    if not os.path.exists(plugin_config.error_path):
        await mk('dir', plugin_config.error_path, mode=None)
    if not os.path.exists(plugin_config.error_path / f"{str(gid)}.json"):
        json_upload(plugin_config.error_path / f"{str(gid)}.json", {str(gid): {time: [module, err]}})
    else:
        try:
            info = json_load(plugin_config.error_path / f"{str(gid)}.json")
            info[str(gid)][time] = [module, err]
            json_upload(plugin_config.error_path / f"{str(gid)}.json", info)
        except Exception as e:
