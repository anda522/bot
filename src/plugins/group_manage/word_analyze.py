...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    with open(plugin_config.word_path, 'r+', encoding='utf-8') as c:
        txt = c.read().split('\n')
        if gid not in txt:
            c.write(gid + '\n')
...
async def _(bot: Bot, event: GroupMessageEvent):
    gid = str(event.group_id)
    txt = plugin_config.word_path.read_text(encoding='utf-8')
    if gid in txt:
        with open(plugin_config.word_path, 'w', encoding='utf-8') as c:
...
async def _(bot: Bot, event: GroupMessageEvent, matcher: Matcher):
    ...
    path_temp = plugin_config.words_contents_path / f"{str(gid)}.txt"
    message_path_group = plugin_config.group_message_data_path / f"{gid}"
    # datetime获取今日日期
    ...
    txt = plugin_config.word_path.read_text(encoding='utf-8').split('\n')
    if gid in txt:
...
async def _(bot: Bot, event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    添加停用词
    """
    await add_txt_line(plugin_config.stop_words_path, matcher, event, args, '停用词')
...
async def _(bot: Bot, event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    删除停用词
    """
    await del_txt_line(plugin_config.stop_words_path, matcher, event, args, '停用词')
...
async def _(bot: Bot, event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    停用词列表
    """
    await get_txt_line(plugin_config.stop_words_path, matcher, event, args, '停用词')
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    更新mask
    """
    already_have = len(os.listdir(plugin_config.wordcloud_bg_path))
    try:
...
async def _(bot: Bot, event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    dic_ = json_load(plugin_config.group_message_data_path / f"{gid}" / f"{today}.json")
...
async def _(bot: Bot, event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    if os.path.exists(plugin_config.group_message_data_path / f"{gid}" / f"{yesterday}.json"):
        dic_ = json_load(plugin_config.group_message_data_path / f"{gid}" / f"{yesterday}.json")
        top = sorted(dic_.items(), key=lambda x: x[1], reverse=True)
        top = (await member_in_group(bot, gid, top))
...
async def _(bot: Bot, event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    gid = event.group_id
    dic_ = json_load(plugin_config.group_message_data_path / f"{gid}" / 'history.json')
    if not dic_:
