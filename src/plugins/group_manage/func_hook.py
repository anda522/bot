...
async def _(matcher: Matcher, bot: Bot, state: T_State, event: Event):
    ...
    if isinstance(event, (
        ...
        gid = event.group_id
        try:
            if which_module in plugin_config.admin_funcs:
                ...
                if not status and which_module not in ['auto_ban',
                    ...
                    if cb_notice:
                        await bot.send_group_msg(group_id=gid,
                                                 message=f"功能处于关闭状态，发送【开关{plugin_config.admin_funcs[which_module][0]}】开启")
...
async def check_func_status(func_name: str, gid: str) -> bool:
    ...
    if funcs_status is None:
        raise FileNotFoundError(plugin_config.switcher_path)
    try:
