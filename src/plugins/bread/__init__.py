...
async def _(event: Event, bot: Bot, args: Message = CommandArg(), cmd: Message = RawCommand()):
    ...
    if wait_time > 0:
        ...
        msg_txt = f"您还得等待{wait_time}秒才能买{thing}w，现在一共拥有{data.bread_num}个{thing}！您的{thing}排名为:{data.no}"
    elif wait_time < 0:
...
async def _(event: Event, bot: Bot, args: Message = CommandArg(), cmd: Message = RawCommand()):
    ...
    if wait_time > 0:
        data = BreadDataManage(group_id).get_bread_data(user_qq)
        msg_txt = f"您还得等待{wait_time}秒才能吃{thing}w，现在你的等级是Lv.{data.bread_eaten // LEVEL}！您的{thing}排名为:{data.no}"
    elif wait_time < 0:
...
async def _(bot: Bot, event: Event, args: Message = CommandArg(), cmd: Message = RawCommand()):
    ...
    if not robbed_qq:
        if bread_config.is_random_robbed:
            ...
            if not all_qq:
                await bot.send(event=event, message="没有可以抢的人")
                return
    ...
    if wait_time > 0:
        msg_txt = f"您还得等待{wait_time}秒才能抢{thing}"
    elif wait_time < 0:
...
async def _(bot: Bot, event: Event, args: Message = CommandArg(), cmd: Message = RawCommand()):
    ...
    if not given_qq:
        if bread_config.is_random_given:
            ...
            all_qq = [x.user_id for x in all_data if x.bread_num and x.user_id != user_qq]
            if not all_qq:
                await bot.send(event=event, message="没有可以赠送的人")
                return
    ...
    if wait_time > 0:
        msg_txt = f"您还得等待{wait_time}秒才能送{thing}w"
    elif wait_time < 0:
...
async def _(bot: Bot, event: Event, args: Message = CommandArg(), cmd: Message = RawCommand()):
    ...
    if wait_time > 0:
        msg_txt = f"您还得等待{wait_time}秒才能猜拳"
